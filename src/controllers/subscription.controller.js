...
const subscribe = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'OTP sent to your email. Please verify to complete your subscription.' });
  } catch (error) {
    console.error(error);
...
const verifyOtp = async (req, res) => {
  ...
  try {
    const tempSub = await SubscriptionTemp.findOne({ where: { email, otp } });
    ...
    // Move to permanent subscribers table
...
const unsubscribe = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ message: 'Something went wrong while unsubscribing.', error: error.message });
  }
};
